      ...
      <div className="mt-10">
        ...
        <ul className="mt-3 space-y-2">
          {CERTIFICATIONS.map((cert) => (
            <li
              key={cert.name}
              className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-0.5 border-t border-border pt-2 text-sm first:border-t-0 first:pt-0"
            >
              <span className="text-ink">{cert.name}</span>
              <span className="font-mono text-xs text-faint">{cert.period}</span>
            </li>
          ))}
        </ul>
      </div>
